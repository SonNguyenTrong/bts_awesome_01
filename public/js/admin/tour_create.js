    ...
    methods: {
        ...
        removeDay(index) {
            ...
            axios.delete('/admin/days/' + i).then((response) => {
                var image = this.dayService.images
                var service = this.dayService.services
                image.forEach(function(key) {
                    debugger
                    self.removeImages(key)
                });
                service.forEach(function(key) {
                    self.removeServices(key)
                });
        ...
        removeImages(index) {
            var self = this
            var i = this.dayService.images[index]
            axios.delete('/admin/images/' + i).then((response) => {
            }).catch((error) => {
                self.dayErrors = error.response.data.errors
            });
        },
        removeServices(index) {
            var self = this
            var i = this.dayService.services[index]
            axios.delete('/admin/services/' + i).then((response) => {
            }).catch((error) => {
                self.dayErrors = error.response.data.errors
            });
        },
